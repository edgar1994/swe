    ...
    public void validate(final FacesContext context, final UIComponent component, final Object value)
        ...
        if (StringUtils.isEmptyOrNullOrBlank(lastname)) {
            message = new FacesMessage(FacesMessage.SEVERITY_ERROR,
                    this.messageService.getMessage("USER.VALIDATOR.TITLE.SUMMARY"),
                    this.messageService.getMessage("USER.VALIDATOR.TITLE.DETAIL.NOTNULL"));
            throw new ValidatorException(message);
        } else if (lastname.length() < 3 || lastname.length() > 30) {
            message = new FacesMessage(FacesMessage.SEVERITY_ERROR,
                    this.messageService.getMessage("USER.VALIDATOR.TITLE.SUMMARY"),
                    this.messageService.getMessage("USER.VALIDATOR.TITLE.DETAIL.LENGTH",
                            lastname.length()));
            throw new ValidatorException(message);
        }
    }
